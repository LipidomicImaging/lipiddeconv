...
def main(args):
    ...
    case,actual=v.mismatch_case(context,parent,targets,'MILD__CAL_R1_K125')
    bound=audit['datasets']['MILD__CAL_R1_K125'];actual=v.v57.canonical(actual)
    # CPU thread count changes BLAS reduction roundoff in these diagnostics only.
    # Do not modify the scientific runner or allow any array/hash change.
    reduction_checks={}
    for key,expected in bound.items():
        if key in ('forward_mismatch_residual','sentinel_gate_residual'):
            assert abs(actual[key]-expected)<=1e-12
            reduction_checks[key]=dict(frozen=expected,recomputed=actual[key],abs_difference=abs(actual[key]-expected))
    ...
    assert v.array_sha(targets['MILD'])==design['scientific']['target_library_hashes']['A_target_MILD_sha256']
    ...
    def fit(matrix,b,method):
        ...
        if method=='scipy_lsq_linear_bvls_tol_1e-12':
            ...
            raw_min=float(np.min(result.x))
            assert raw_min>=-1e-12*max(float(np.max(np.abs(result.x))),1.),(method,raw_min)
            # BVLS may accept an almost feasible unconstrained initial solution.
            # Record and project only tiny negative roundoff; evaluate the feasible fit.
            x=np.maximum(result.x,0.);projection_norm=float(np.linalg.norm(x-result.x))
            residual=float(np.linalg.norm(matrix@x-b))
        elif method=='scipy_nnls_original_order':
            x,residual=nnls(matrix,b,maxiter=plan['maxiter'])
        else:
        ...
        diagnostics=dict(seconds=time.monotonic()-start,returned_residual=float(residual),
            raw_min_coefficient=raw_min,negative_roundoff_projection_norm=projection_norm,
        ...
        for domain,b in signals.items():
            ...
            for method in methods:
                ...
                row=dict(**selected,domain=domain,method=method,q_full=fd['returned_residual']**2,q_deleted=dd['returned_residual']**2,
                    ...
                    full_raw_min_coefficient=fd['raw_min_coefficient'],deleted_raw_min_coefficient=dd['raw_min_coefficient'],
                    full_negative_projection_norm=fd['negative_roundoff_projection_norm'],deleted_negative_projection_norm=dd['negative_roundoff_projection_norm'],
    ...
    write(args.output/'provenance.json',dict(status='COMPLETE_AWAITING_REVIEW',selection_sha256=sha(args.selection),
        ...
        no_HOLD_outcomes_parsed=True,no_GPU_training=True,known_mismatch_is_not_measurement_noise=True,
        derived_reduction_checks=reduction_checks,all_scientific_array_hashes_exact=True,
